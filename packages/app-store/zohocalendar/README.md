...
2. Create a "Server-based Applications", set the Redirect URL for OAuth `<MeetSynq URL>/api/integrations/zohocalendar/callback` replacing MeetSynq URL with the URI at which your application runs.
3. Fill in any information you want in the "Client Details" tab
4. Go to tab "Client Secret" tab.
5. Now copy the Client ID and Client Secret into your app keys in the MeetSynq admin panel (`<MeetSynq>/settings/admin/apps`).
...
9. You're good to go. Now you can easily add your Zoho Calendar integration in the MeetSynq settings at `/settings/my-account/calendars`.
